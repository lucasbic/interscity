...
To help you start experimenting with the InterSCity platform, we provide a sample configuration for 5 virtual machines, comprising a [Vagrantfile](https://www.vagrantup.com/) that automatically creates the VMs and an [ansible hosts file](ansible/vagrant_hosts). They are also good templates to start from if you want to define a different setup.

When using this sample configuration, you do not need to worry about adding your public key to the VMs; vagrant adds a default key for you (the corresponding private key is at `~/.vagrant.d/insecure_private_key`). With it, you may access the machines with `vagrant ssh macine-name`. The provided ansible scripts also depend on this default key (it is hardcoded in the sample hosts file). However, keep in mind that *this is insecure* and only suitable for local access.
...
Instead of real hosts, you may deploy InterSCity locally on virtual
machines with vagrant and virtualbox. The provided `Vagrantfile`
describes 5 virtual machines and the provided `ansible/vagrant_hosts`
may be used as ansible's `hosts` file to make it work with those
machines. You also may install all InterSCity services on a single
virtual machine. To do that, use `ansible/standalone_vagrant_host`
as ansible's `host` file and modify the Vagrantfile so as to (1)
increase the memory to `2048` or even `4096` and (2) remove the
unnecessary VMs, leaving just the `gateway-machine` definition.
...
The provided files use different addresses, so you should either change
them or modify your machine's virtualbox configuration:
...
After all these steps, enter the `ansible` directory and run:
